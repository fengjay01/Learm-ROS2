...
def generate_launch_description():
    ...
    dummy_moveit_config = FindPackageShare(
        package='learm_moveit_config').find('learm_moveit_config')
    ...
    load_joint_trajectory_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller',
             '--set-state', 'active', 'learm_arm_controller'],
        output='screen'
    )
    ...
    load_joint_trajectory_controller_gripper = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller',
             '--set-state', 'active', 'learm_hand_controller'],
        output='screen'
    )
